...
func CreateAccountModel(name string, email string, password string) (*AccountModel, error) {
	...
	if err := accountModel.SetEmail(email); err != nil {
		return nil, err
	}
...
func (m *AccountModel) SetEmail(email string) error {
	if len(email) < 1 {
		return application_error.ERR_REQUIRED_ACCOUNT_EMAIL
	}
