...
func (s *accountService) Create(ctx context.Context, request *payload.RegisterRequest) (*payload.AccountInfo, error) {
	existingAccount, err := s.Repository.Account.SelectByUsername(ctx, request.Username)
	if err != nil && !errors.Is(err, application_error.ERR_NOT_FOUND_ACCOUNT_USERNAME) {
		return nil, err
	}

	if existingAccount != nil {
		return nil, application_error.ERR_ALREADY_USED_ACCOUNT_USERNAME
	}

	account, err := model.CreateAccountModel(request.Name, request.Username, request.Password)
	if err != nil {
		return nil, err
	}
...
func (s *accountService) Login(ctx context.Context, request *payload.LoginRequest) (*payload.AuthenticationTokenResponse, error) {
	account, err := s.Repository.Account.SelectByUsername(ctx, request.Username)
