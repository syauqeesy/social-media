...
func Test_CreateAccountModel(t *testing.T) {
	// success
	user, err := CreateAccountModel("name", "Username", "password")
	assert.Nil(t, err)
	assert.NotNil(t, user)

	// failed: user name required
	user, err = CreateAccountModel("", "Username", "password")
	assert.Equal(t, application_error.ERR_REQUIRED_ACCOUNT_NAME, err)
	assert.Nil(t, user)

	// failed: name cannot be more than 191 characters
	user, err = CreateAccountModel("Lorem ipsum dolor sit amet, consectetur adipiscing elit. Mauris pharetra enim imperdiet, consectetur felis in, pharetra nisi. Curabitur non ex pellentesque, imperdiet nisl at, finibus mauris. Curabitur eu tempus nisi.", "Username", "password")
	assert.Equal(t, application_error.ERR_MAXIMUM_LENGTH_ACCOUNT_NAME, err)
	assert.Nil(t, user)

	// failed: Username is required
	user, err = CreateAccountModel("name", "", "password")
	assert.Equal(t, application_error.ERR_REQUIRED_ACCOUNT_USERNAME, err)
	assert.Nil(t, user)

	// failed: minimum length for password is 8 characters
	user, err = CreateAccountModel("name", "Username", "short")
...
func Test_AccountModel_SetUsername(t *testing.T) {
	var err error
